...
const getlastAntrian = async function () {
    ...
    let sql = `SELECT antrian_loket .nomor_antri, antrian_loket .created_at, antrian_loket .uid FROM antrian_loket WHERE antrian_loket .created_at LIKE ? ORDER BY  antrian_loket .nomor_antri DESC LIMIT 1`
...
const getSisaAntrian = async function () {
    let date = new Date()
    let date_now = "%" + date.getFullYear() + "-" + ("0" + (date.getMonth() + 1)).slice(-2) + "-" + ("0" + (date.getDate())).slice(-2) + "%";
    let sql = `SELECT COUNT('sisa') AS sisa FROM  antrian_loket  WHERE  antrian_loket .created_at LIKE ? AND  antrian_loket .updated_at IS NULL`
    const result = await con.query(sql, [date_now]);
...
const postAntrian = async function (nomor_antri) {
    ...
    let sql = `INSERT INTO antrian_loket (nomor_antri, created_at) VALUES (?, ?)`
...
const updateAntrian = async function (loket, uid) {
    ...
    let sql = `UPDATE antrian_loket SET updated_at = ?, loket = ? WHERE uid = ?`
